...
const addCustomTag = () => {
  ...
  const customTagUpperCase = customTag.charAt(0).toUpperCase() + customTag.slice(1);
  const existedTagSearchListUpperCase = existedTagSearchList.value.map(tag => tag.charAt(0).toUpperCase() + tag.slice(1));
  if (toUpperCaseTags.includes(customTagUpperCase) || existedTagSearchListUpperCase.includes(customTagUpperCase)) {
    showFailToast("该标签已存在");
    return;
  }
  if (existedTagSearchList.value.length >= 12) {
    showFailToast("最多只能选择12个标签");
    return;
  }
  existedTagSearchList.value.push(customTagUpperCase);

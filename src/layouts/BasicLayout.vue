<template>
  <van-nav-bar v-if="!NOT_SHOW_NAVBAR_AND_TABBAR.includes(route.path)" fixed="fixed" left-arrow left-text="返回"
    ...
    <template v-else-if="route.path === '/blog'" #right>
      <van-badge :content="blogNoticeCount" v-if="blogNoticeCount>0">
        <notice/>
      </van-badge>
    </template>
  ...
  <van-tabbar v-model="active" v-if="!NOT_SHOW_NAVBAR_AND_TABBAR.includes(route.path)" @change="onChange" route>
    ...
    <van-tabbar-item icon="records" name="blog" replace to="/blog">博客</van-tabbar-item>
...
const recordId = ref([])
const blogNoticeCount = ref(0)
const blogRecordId = ref([])
